...
app.post('/api/Assignmentshow', (req, res) => {
  const sql = 'SELECT * FROM assignment,student where assignment.SID=student.Sid';
  ...
  db.query(sql, (err, results) => {
	...
	const formattedDate = `${date.getDate()}-${date.getMonth() + 1}-${date.getFullYear()}`;
      ...
      return `<tr><td>${row.EID}</td><td>${row.SID}</td><td>${formattedDate}</td><td>${row.Name}</td><td>${row.PRN}</td><td>${row.Department}</td><td>${row.Year}</td><td><a href="http://localhost:3001/uploads/${row.Filepath}">Download</a></td><td>${row.Adate}</td></tr>`;
      // Replace 'column1', 'column2', etc. with your actual column names
    });
...
app.post('/api/StudentExprimentshow', upload.none(), (req, res) => {
  ...
  db.query(sql, values, (err, results) => {
	...
	// Format results into an HTML table
    const tableRows = results.map(row => {
      return `<h1>${row.EID}.${row.ExperimentTitle}</h1>
...
// Define an API endpoint to insert data
app.post('/api/ExperimentAssignment', upload.single('file'), (req, res) => {
  ...
  const sql = 'INSERT INTO assignment(EID,SID,Filepath) VALUES (?, ?, ?)';
  const values = [EID, SID,req.file.filename];
